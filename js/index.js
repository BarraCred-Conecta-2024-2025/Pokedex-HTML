...
window.onscroll = () => {
    if (((window.innerHeight + window.scrollY) >= document.body.scrollHeight - 500) && !loading && !filtered) {
        topo += 40;
        loadPokemons();
    }
}
...
//<img src="${pokemon.sprites.other['official-artwork'].front_default}" alt="${pokemon.name}" class="card-img-top"></img>



// console.log(await getData('pokemon/1'));
// let description = await getData('pokemon-species/1');
// //console.log(description.flavor_text_entries);
// let vetor = Array.from(description.flavor_text_entries);
// console.log(vetor.filter(item => item.language.name == 'en')[4].flavor_text.replace('\f', ' '));
